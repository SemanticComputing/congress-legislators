...
def main():
    infilename = "../csv/legislators-district-offices.csv"
    num_uri = 1
    
    
    writeRDF(infilename,
            './legislators-district-offices-template.ttl',
            num_uri,
            '../ttl/legislators-district-offices.ttl')
...
def writeRDF(infilename, templatefilename, num_uri, outfilename):
    ...
    for row in reader:
        ...
        if "gender" in row:
            y = row["gender"]
            if y =="F": row["gender"] = "Female"
            if y =="M": row["gender"] = "Male"
            # todo 'suku', 'pariskunta'
      
        if "type" in row:
            y = row["type"]
            if y =="sen": row["type"] = "Senator"
            if y =="rep": row["type"] = "Representative"

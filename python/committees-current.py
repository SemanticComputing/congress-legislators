...
def main():
    infilename = "../csv/committees-current.csv"
    num_uri = 1
    
    
    writeRDF(infilename,
            './committees-current-template.ttl',
            num_uri,
            '../ttl/committees-current.ttl')
...
def writeRDF(infilename, templatefilename, num_uri, outfilename):
    ...
    for row in reader:
        ...
        if "terms__type" in row:
            y = row["terms__type"]
            if y =="prez": row["terms__type"] = "President"
            if y =="viceprez": row["terms__type"] = "Vice President"
